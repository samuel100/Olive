...
from olive.workflows.run.config import OliveConfig, RunConfig, RunPassConfig

logger = logging.getLogger(__name__)


def dependency_setup(olive_config: OliveConfig, run_config: RunConfig):
    extras = deepcopy(olive_config.extra_dependencies)
    ...
    def get_pass_extras(pass_type):
        olive_pass_config = olive_config.passes.get(pass_type)
        pass_to_extra = olive_pass_config.module_dependencies
        pass_to_extra_names = olive_pass_config.extra_dependencies
...
def run_engine(olive_config: OliveConfig, run_config: RunConfig, data_root: str = None):
    ...
    for accelerator_spec, (passes, pass_flows) in zip(acc_list, pass_list):
        ...
        if passes:
            ...
            for pass_config in passes.values():
                olive_pass_config = olive_config.passes.get(pass_config.type)
                if olive_pass_config:
                    logger.info("Importing module %s", olive_pass_config.module_path)
                    module_path, class_name = olive_pass_config.module_path.rsplit(".", 1)
                    importlib.import_module(module_path, class_name)
                else:
                    raise ValueError(f"Configuration for pass of type '{pass_config.type}' not found")
...
def run(
    ...
    setup: bool = False,
    data_root: str = None,
    olive_config: Union[str, Path, dict] = None,
):
    if olive_config is None:
        olive_config = OliveConfig.get_default_config_path()

    # we use parse_file and parse_obj to be safe. If implemented as expected, both should be equivalent.
    logger.info("Loading Olive configuration: %s", olive_config)
    if isinstance(olive_config, (str, Path)):
        olive_config = OliveConfig.parse_file(olive_config)
    else:
        olive_config = OliveConfig.parse_obj(olive_config)
    ...
    if setup:
        ...
        set_verbosity_info()
        dependency_setup(olive_config, run_config)
        return None
    else:
        return run_engine(olive_config, run_config, data_root)
